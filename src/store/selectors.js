...
const buildGraphData = (orders) => {
  ...
  const graphData = hours.map((hour) => {
    ...
    open = parseFloat(open.tokenPrice).toFixed(2);
    high = parseFloat(high.tokenPrice).toFixed(2);
    low = parseFloat(low.tokenPrice).toFixed(2);
    close = parseFloat(close.tokenPrice).toFixed(2);
    return({
      x: new Date(hour),
